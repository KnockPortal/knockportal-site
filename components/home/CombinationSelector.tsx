...
function TradeRow({ trade, state }: { trade: CombinationTrade; state: RowState }) {
  switch (state) {
    case 'available':
      return (
          ...
          <li key={city.slug}>
            {city.filled ? (
              <>
                ...
                <ul className="divide-y divide-hairline border-t border-hairline">
                  {city.trades.map((trade) => (
                    <li key={trade.slug}>
                      <TradeRow trade={trade} state={rowState(trade, dataDown)} />
                    </li>
                  ))}
                </ul>
              </>
              ...
              <button
                type="button"
                disabled
                className={cn(ROW_BASE, 'font-display text-base font-semibold', ROW_DISABLED)}
              >
