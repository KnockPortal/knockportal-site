...
- В `.eslintrc.json` правило `@next/next/no-sync-scripts` выключено для группы
  `app/(surface)/**`. Причина: порядок загрузки ассетов поверхности намеренно синхронный
  и задокументирован в самом файле маршрута. Область взята по группе, а не по одному
  файлу, потому что `[city]` и `[trade]` для minimatch — символьные классы, и точечное
  экранирование хрупко.
